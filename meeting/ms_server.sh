...
CONTAINER_NAME=${_local_domain_name//./_}_ms
...
echodo podman rmi -f ms_server:latest
...
cat <<EOF
traefik.enable=true
traefik.http.routers.${CONTAINER_NAME}.rule=Host(\`${_local_domain_name}\`) && PathPrefix(\`/ws/protoo/ms\`)
...
traefik.http.services.srv_${CONTAINER_NAME}.loadbalancer.server.port=4100
EOF
}
...
-v ${DATA_DIR}:/data \
-e MEDIASOUP_ANNOUNCED_IP=${_local_domain_name} \
-p 4100-4120:4100-4120/udp \
-p 4100-4120:4100-4120/tcp \
--label-file <(__label_file) \
docker://registry.rbat.tk/ms_server:latest
